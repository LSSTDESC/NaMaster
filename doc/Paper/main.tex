...
\maketitlepost
  ...
  \section{The pseudo-\texorpdfstring{$C_\ell$}{Cl} estimator}\label{sec:maths}
    ...
    \subsection{Spherical harmonic transform and pseudo-\texorpdfstring{$C_\ell$}{Cl}s}\label{ssec:maths.pcl101}
      ...
      \subsubsection{Power-spectrum estimation}
        The problem we are attempting to solve is how to estimate ${\sf C}^{ab}_\ell$ from a single realization of ${\bf a}$ and ${\bf b}$ measured on a cut sky. In general, the input data for measuring the power spectrum come pixelized on a sphere. Instead of dealing with an infinitely sampled points, we have a finite number of measurements
        \begin{equation}
  ...
  \section{Code structure}\label{sec:structure}
    All of the methods described above have been implemented in a software package called \nmt\footnote{The code source is hosted in \url{add_url}, and its documentation can be found at \url{http://namaster.readthedocs.io/}.}. We briefly describe the code structure here.
...
cl_decoupled=wsp.decouple_cell(cl_coupled,
 ...
 \label{ssec:scalability}
     One of the known advantages of the pseudo-$C_{\ell}$ estimator is that it is usually much faster that other known estimators~\citep{2017MNRAS.465.1847E} since the complexity scales as $\mathcal{O}(\ell_{max}^{3})$, where $\ell_{max}$ is the maximum multipole at which the power-spectra are calculated. In this section we will show that \nmt follows this same scaling with $\ell_{max}$, and we will also study how the code behaves when the number of parallel threads is changed.
     
     We first focus on checking the dependence of the execution time with $\ell_{max}$. We compute the full-sky angular auto and cross-power spectra of a spin-0 and a spin-2 field, with no foreground contamination. We change the resolution of the \texttt{HEALPix} maps by varying the $N_{side}$ parameter in the range [16, 2048], and we calculate the power-spectra up to $\ell_{max}=3N_{side}$. We use one interactive Cori-Haswell node at the National Energy Research Scientific Computing Center (NERSC). This kind of nodes have two 16-core Intel Xeon (E5-2698 v3) processors at 2.3 GHz, and 128 GB DDR4 of RAM. Each core supports two hyper-threads, so we end up executing the code using 64 \texttt{OpenMP} threads. In Figure~\ref{fig:scaling_lmax} we can see the results for this test. We show the results for the auto-power spectra for the spin-0 and spin-2 fields (Data 00, and Data 22, respectively), as well as the results for the cross-power spectra (Data 20). In all cases when we compare with the reference line $\propto \ell_{max}^{3}$ we can see that the code scales as expected. We fixed the proportionality constant for the reference line using the timing of the auto-power of the spin-0 field at $\ell_{max}=96$.  We can also see that for very small $N_{side}$ data handling time starts to be comparable to the pure power-spectra computation time, increasing the expected execution time.
     
     \begin{figure}
     \centering
     \includegraphics[width=0.9\columnwidth]{time_scaling}
     \caption{Computing time for the auto-power spectra of full-sky spin-0 (data 00, hollow circles), and spin-2 (data 22, solid triangles) foreground-free maps, as wells as their cross-power (data 02, solid circles) as a function of $\ell_{max}$. We include the expected scaling for the pseudo-$C_{\ell}$ estimator (solid line) and force it to go through the point for the spin-0 auto-power at $\ell=96$ showing remarkable agreement.}
     \label{fig:scaling_lmax}
     \end{figure}
     
  \todo{Scaling with number of threads}   
