...
static void purify_generic_CAR(nmt_field_CAR *fl,flouble *mask,fcomplex **walm0,
  flouble **maps_in,fcomplex **alms_out)
{
  if(fl->pure_b || fl->pure_e) {
    nmt_purify_CAR(fl,mask,walm0,maps_in,maps_in,alms_out);
  }
  else {
    int im1;
    for(im1=0;im1<fl->nmaps;im1++)
      rect_map_product(fl->cs,maps_in[im1],mask,maps_in[im1]);
    rect_map2alm(fl->cs,fl->lmax,1,2*fl->pol,maps_in,alms_out,HE_NITER_DEFAULT);
  }
}
...
// -------------------------- CAR ----------------------
static nmt_workspace_CAR *nmt_workspace_CAR_new(nmt_curvedsky_info *cs,
  int ncls,nmt_binning_scheme *bin,int is_teb)
{
  int ii;
  nmt_workspace_CAR *w=my_malloc(sizeof(nmt_workspace_CAR));
  w->lmax=bin->ell_max;
  w->is_teb=is_teb;
  w->ncls=ncls;
...
static void bin_coupling_matrix_CAR(nmt_workspace_CAR *w)
{
  int icl_a,icl_b,ib2,ib3,l2,l3,i2,i3,sig;
...
// fl1,fl2 (in) : fields we're correlating
// coupling_matrix_out (out) : unbinned coupling matrix
nmt_workspace_CAR *nmt_compute_coupling_matrix_CAR(
  nmt_field_CAR *fl1,nmt_field_CAR *fl2,nmt_binning_scheme *bin,int is_teb)
{
  int l2;
  nmt_workspace_CAR *w;
  flouble *beam_prod;
  ...
  if(diff_curvedsky_info(fl1->cs, fl2->cs))
    report_error(NMT_ERROR_CONSISTENT_RESO,"Can't correlate fields with different resolutions\n");
  if(bin->ell_max>=3*fl1->cs->n_eq)
  {
    printf("%i, %li \n", bin->ell_max, 3*fl1->cs->n_eq);
    report_error(NMT_ERROR_CONSISTENT_RESO,"Requesting bandpowers for too high a multipole given map resolution\n");
  }
  w=nmt_workspace_CAR_new(fl1->cs,n_cl,bin,is_teb);
  beam_prod=my_malloc((w->lmax+1)*sizeof(flouble));
  memcpy(w->mask1,fl1->mask,w->cs->npix*sizeof(flouble));
  memcpy(w->mask2,fl2->mask,w->cs->npix*sizeof(flouble));
  rect_anafast(&(fl1->mask),&(fl2->mask),0,0,&(w->pcl_masks),fl1->cs,w->lmax,HE_NITER_DEFAULT);
  for(l2=0;l2<=w->lmax;l2++) {
    w->pcl_masks[l2]*=(2*l2+1.);
    beam_prod[l2]=fl1->beam[l2]*fl2->beam[l2];
  }
...
#pragma omp for schedule(dynamic)
  ...
  bin_coupling_matrix_CAR(w);
  free(beam_prod);

  return w;
}

nmt_workspace_CAR *nmt_compute_power_spectra_CAR(nmt_field_CAR *fl1,
  nmt_field_CAR *fl2,
					 ...
					 flouble **cl_noise,flouble **cl_proposal,flouble **cl_out)
{
  ...
  if(w0==NULL)
    w=nmt_compute_coupling_matrix_CAR(fl1,fl2,bin,0);
  else {
    w=w0;
    if(w->lmax>=3*fl1->cs->n_eq)
      report_error(NMT_ERROR_CONSISTENT_RESO,"Workspace does not match map resolution\n");
  }
  ...
  nmt_compute_coupled_cell_CAR(fl1,fl2,cl_data);
  nmt_compute_deprojection_bias_CAR(fl1,fl2,cl_proposal,cl_bias);
  nmt_decouple_cl_l_CAR(w,cl_data,cl_noise,cl_bias,cl_out);
  for(ii=0;ii<w->ncls;ii++) {
    free(cl_bias[ii]);
    free(cl_data[ii]);
  }
...
void nmt_compute_deprojection_bias_CAR(nmt_field_CAR *fl1,
  nmt_field_CAR *fl2, flouble **cl_proposal,flouble **cl_bias)
{
  int ii;
  flouble **cl_dum;
  long ip;
  ...
  if(fl2->ntemp>0) {
	...
	//w*g^j
	for(im2=0;im2<fl2->nmaps;im2++)
	  rect_map_product(fl2->cs,fl2->temp[itj][im2],fl2->mask,map_2_dum[im2]);
	//SHT[w*g^j]
	rect_map2alm(fl2->cs,fl2->lmax,1,2*fl2->pol,map_2_dum,alm_2_dum,HE_NITER_DEFAULT);
	//C^ab*SHT[w*g^j]
	for(im1=0;im1<fl1->nmaps;im1++) {
	  he_zero_alm(fl1->lmax,alm_1_dum[im1]);
	  for(im2=0;im2<fl2->nmaps;im2++)
	...
	//SHT^-1[C^ab*SHT[w*g^j]]
	rect_alm2map(fl1->cs,fl1->lmax,1,2*fl1->pol,map_1_dum,alm_1_dum);
	//SHT[v*SHT^-1[C^ab*SHT[w*g^j]]]
	purify_generic_CAR(fl1,fl1->mask,fl1->a_mask,map_1_dum,alm_1_dum);
	//Sum_m(SHT[v*SHT^-1[C^ab*SHT[w*g^j]]]*g^i*)/(2l+1)
	he_alm2cl(alm_1_dum,fl2->a_temp[iti],fl1->pol,fl2->pol,cl_dum,lmax);
	for(im1=0;im1<nspec;im1++) {
	  for(ip=0;ip<=lmax;ip++)
  ...
  if(fl1->ntemp>0) {
	...
	//v*f^j
	for(im1=0;im1<fl1->nmaps;im1++)
	  rect_map_product(fl1->cs,fl1->temp[itj][im1],fl1->mask,map_1_dum[im1]);
	//SHT[v*f^j]
	rect_map2alm(fl1->cs,fl1->lmax,1,2*fl1->pol,map_1_dum,alm_1_dum,HE_NITER_DEFAULT);
	//C^abT*SHT[v*f^j]
	for(im2=0;im2<fl2->nmaps;im2++) {
	  he_zero_alm(fl2->lmax,alm_2_dum[im2]);
	  for(im1=0;im1<fl1->nmaps;im1++)
	...
	//SHT^-1[C^abT*SHT[v*f^j]]
	rect_alm2map(fl2->cs,fl2->lmax,1,2*fl2->pol,map_2_dum,alm_2_dum);
	//SHT[w*SHT^-1[C^abT*SHT[v*f^j]]]
	purify_generic_CAR(fl2,fl2->mask,fl2->a_mask,map_2_dum,alm_2_dum);
	//Sum_m(f^i*SHT[w*SHT^-1[C^abT*SHT[v*f^j]]]^*)/(2l+1)
	he_alm2cl(fl1->a_temp[iti],alm_2_dum,fl1->pol,fl2->pol,cl_dum,lmax);
	for(im1=0;im1<nspec;im1++) {
	  for(ip=0;ip<=lmax;ip++)
  ...
  if((fl1->ntemp>0) && (fl2->ntemp>0)) {
	...
	//w*g^q
	for(im2=0;im2<fl2->nmaps;im2++)
	  rect_map_product(fl2->cs,fl2->temp[itq][im2],fl2->mask,map_2_dum[im2]);
	//SHT[w*g^q]
	rect_map2alm(fl2->cs,fl2->lmax,1,2*fl2->pol,map_2_dum,alm_2_dum,HE_NITER_DEFAULT);
	//C^ab*SHT[w*g^q]
	for(im1=0;im1<fl1->nmaps;im1++) {
	...
	for(im1=0;im1<fl1->nmaps;im1++) {
	  //v*SHT^-1[C^ab*SHT[w*g^q]]
	  rect_map_product(fl1->cs,map_1_dum[im1],fl1->mask,map_1_dum[im1]);
	  //Int[f^jT*v*SHT^-1[C^ab*SHT[w*g^q]]]
	  mat_prod[itj*fl2->ntemp+itq]+=rect_map_dot(fl1->cs,map_1_dum[im1],fl1->temp[itj][im1]);
	}
      }
    }
