...
def get_sample_field() :
    mp_t,mp_q,mp_u=hp.synfast([cl_tt,cl_ee,cl_bb,cl_te],nside,verbose=False)
...
cw.compute_coupling_coefficients(f0,f0,f0,f0) #<- This is the time-consuming operation
...
covar_02_02=nmt.gaussian_covariance(cw,0,2,0,2, #Spins of the 4 fields
                                    ...
                                    w02,wb=w02).reshape([n_ell,2,n_ell,2])
...
covar_00_22=nmt.gaussian_covariance(cw,0,0,2,2, #Spins of the 4 fields
                                    ...
                                    [cl_te,cl_tb], #TE,TB
                                    [cl_te,cl_tb], #TE,TB
                                    w00,wb=w22).reshape([n_ell,1,n_ell,4])
...
covar_02_22=nmt.gaussian_covariance(cw,0,2,2,2, #Spins of the 4 fields
                                    ...
                                    [cl_ee,cl_eb,cl_eb,cl_bb], #EE,EB,BE,BB
                                    [cl_ee,cl_eb,cl_eb,cl_bb], #EE,EB,BE,BB
                                    w02,wb=w22).reshape([n_ell,2,n_ell,4])
...
#Let's now compute the sample covariance (we'll only do this for spin-0 for simplicity
...
#Let's plot them:
plt.figure(); plt.imshow(covar_TT_TT,origin='lower',interpolation='nearest')
plt.figure(); plt.imshow(covar_sample,origin='lower',interpolation='nearest')
plt.figure(); plt.imshow(covar_TT_TT-covar_sample,origin='lower',interpolation='nearest')
plt.show()
