...
class NmtCovarianceWorkspace(object):
    ...
    def compute_coupling_coefficients(self, fla1, fla2, flb1=None, flb2=None, lmax=None, n_iter=3):
        """
        ...
        self.wsp = lib.covar_workspace_init_py(fla1.fl,fla2.fl,flb1.fl,flb2.fl,lmax,n_iter)
...
def gaussian_covariance(cw, spin_a1, spin_a2, spin_b1, spin_b2, 
    ...
    if ((len(cla1b1) != nm_a1*nm_b1) or (len(cla1b2) != nm_a1*nm_b2) or
        (len(cla2b1) != nm_a2*nm_b1) or (len(cla2b2) != nm_a2*nm_b2)) :
        raise ValueError("Input spins do not match input power spectrum shapes")

    if (
        (len(cla1b1[0]) < cw.wsp.lmax + 1)
        or (len(cla1b2[0]) < cw.wsp.lmax + 1)
        or (len(cla2b1[0]) < cw.wsp.lmax + 1)
        or (len(cla2b2[0]) < cw.wsp.lmax + 1)
    ):
...
def gaussian_covariance_flat(cw, spin_a1, spin_a2, spin_b1, spin_b2, larr,
    ...
    covar1d = lib.comp_gaussian_covariance_flat(
        cw.wsp, spin_a1, spin_a2, spin_b1, spin_b2,
        wa.wsp, wb.wsp, larr, cla1b1, cla1b2, cla2b1, cla2b2, len_a * len_b
    )
    covar = np.reshape(covar1d, [len_a, len_b])
    return covar
